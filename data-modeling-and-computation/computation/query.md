...
和迭代器一样，我们可以对query进行transform。比如mapping，filtering。transform的query。query作为抽象的dataview，transformation在access和迭代时当场计算，而不是转化原始数据。
...
- 常见的集合运算 `A: Query<K1, V1>`，`B: Query<K2, V2>` 采用不同的集合运算规则组合出 `Query<(K1, K2), (V1, V2)>`
...
增量计算是面向变更数据的计算，关于数据的变更，通过这样的枚举表达：
...
定义DeltaQuery为 value是valuechange的Query，`DeltaQuery<K, V>` 即： `Query<K, ValueChange<V>>`。一般称普通的Query为全量表，DeltaQuery为增量表
...
Query是一个抽象kv的概念，表达了k到v的一一对应关系。在某些情况下，我们需要抽象的一对多关系查询，所以类似的，我们设计MultiQuery的抽象：
...
和Query一样，只不过因为对应多个Value。所以返回Value的迭代器。
和Query一样，也可以写一些比如map的operator

### TriQuery

如果我们能够同时拥有 `Query<K, V>`, `DeltaQuery<K, V>`,(这两项即`DualQuery<K, V>`)，再加上 `MultiQuery<V, K>`(注意这边是反的)。那么我们将这三者组合起来，称之为`TriQuery<K, V>`

`TriQuery<K, V>`是一个有用的常见结构，其主要作用是实现不同K的映射。具体来说可以实现以下两种增量算子：

- one-to-many Fanout： `(DualQuery<K1, V>, TriQuery<K1, K2>) ⇒ DualQuery<K2, V>`
- many-to-one Reduce(simple):  `(DualQuery<K2, ()>, TriQuery<K1, K2>) ⇒ DualQuery<K1, ()>`

这两个算子特别是fanout，是非常常用的。它增量实现了不同的具有一对多关系的K的query，他们之间的进行转换的增量计算。只要用户能够提供K之间的triquery关系。

不同K的query转化是常见的，比如我有一个A类型的entity会引用B类型的entity（多个B可能被一个A引用），同时我有B类型的为K的某个数据，比如`Query<Kb, X>`, 那么我watch Ka Kb来获得 `TriQuery<Ka, Kb>`，那么我就可以拿到`Query<Ka, X>`，相当于我把Kb的数据，通过AB之间的多对一关系，fanout到了Ka上。

我们通过relational DB来建模数据时，不同的数据被不同的entity（Key）来组织，并且不同的entity之间通过外键相连，fanout和reduce实现了不同entity之间的数据查询工作，实际上是一种考虑了多对多关系，特别实现的多表incremental join。
...
用户组装的Dualquery算子时，需要传入Context。在实际change发生后，负责生成delta query的channel实现会wake，以此实现reactive能力的支持。用户可以直接根据wake信息，在最上层或者任意层次结构来直接得知是否存在增量change需要消费，以避免任何check消费的固定成本，或根据change信息实现任何额外逻辑。
...
用户构造的计算图可能并不是被完全消费的，可能出现多阶段消费的情况。

假设考虑有A B两个feature，这两个feature可以share部分的计算结果，以及数据源。但是这两个feature的消费并不会同时发生。 一个典型的例子，比如一个3d场景的pick和rendering：我们允许用户任何时刻都修改场景，然后执行pick操作（消费场景变更，维护支持pick能力的相关数据），同时在每一帧会触发一次render（消费场景变更，维护支持渲染能力的相关数据）。

我们可以简单的假设计算图必须整体一起消费，那么对于上述的case。picking的消费会同时完成render的消费，这导致picking的延迟增加（当然渲染的延迟减少）。并且由于场景的变更可能相互抵消，所以提早消费render有可能造成额外工作。这个问题可能并不是非常严重，但的确说明了这种简单消费模型的某种缺陷。而且要求计算图必须整体一起消费，这样的限制对用户来说其实是不直观的。

所以计算图需要支持多阶段消费的消费行为。支持这样的行为，需要考虑的一个重要问题是，计算图中的分叉节点（多下游节点）可能会出现丢失变更的情况。因为变更信息在分叉节点，需要广播到所有下游，而多阶段消费的某一次消费过程中，可能只会消费一部分的下游，没有被消费到的下游节点，需要保存变更，而不是丢弃变更。所以要支持多阶段消费，需要在分叉节点的每一个下游都维护历史变更的队列，来保证变更的完整性。

query的定义是一个map，所以k是不可重复的。所以在分叉节点的下游进行消费时，需要将新的变更，和历史变更队列的所有变更，按照发生的历史顺序进行合并。ValueChange是可以合并，所以deltaquery是可以合并的。这样的合并会导致性能开销，所以支持计算图的多阶段消费可能并不带来性能上的优势。另一个细节是，不确定的消费行为可能会导致历史消费队列保存unbound的变更信息，导致内存使用增加。解决这个问题需要上游在广播变更时就识别这种情况并直接对buffer过多变更的下游执行历史消息合并。
