...
假设我们考虑对DeltaQuery执行mapping，filter之类的transform，是否可以完成数据集的增量消费？
...
这些组合子的实现内部常也会使用到普通query的组合子。Dualquery组合子的实现是比较复杂的，不过其复杂度被Dualquery这个结构完美封装了。用户实现增量消费的过程，就是去组装dualquery计算，然后直接访问output的增量表即可获得transform后的change。因为dualquery组合子的实现保证了消费成本完全正比于增量表大小，无论Dualquery是由多么复杂的实现组装出来，都可以保证最终消费成本完全正比于增量表。
