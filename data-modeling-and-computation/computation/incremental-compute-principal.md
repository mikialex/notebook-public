...
增量计算是一类至关重要的性能优化，任何富状态小变更特征的系统，比如UI框架，实际上都是增量计算框架。本文尝试在不涉及任何具体实现的前提下，给出其最一般的原理分析。
...
Cache（中间计算结果的缓存）对于增量计算来说并不是必需的，是否需要cache取决于计算的依赖结构。比如一个计算图中，每一个input节点都只对应一个output节点，我们我们就完全不需要cache来控制变更传播的范围，依然能实现有效的增量计算。
