...
[rendiation](https://github.com/mikialex/rendiation)是我在19年12月14日创建的side project，距今天约整六年。做一个简单的回顾，并总结一下这六年来在这个项目上工作的感受。

## 不断进化的 ship of thesues

最早开这个坑的背景是：我在18年初毕业实习开始做webgl的渲染优化，到19年底时我的工作让我对web上做基础软件开发的合理性充满质疑，所以19年底开始我试图尝试做一些非web上的渲染实现。

这6年里我提交了超过5k+ commit。每一年都有重大的技术决策变更和重构。根据工具统计，今天写的代码，一年后平均只会存留一半。现在和几年前的codebase从实现角度看几乎不是一个项目。

我喜欢一个务实的工程方法论：先写一个垃圾出来。
...
好的技术是生长出来的，是从一个垃圾的状态迭代改进而来的。不存在被规划出来的好的结果。 从具体的场景，推导出必要的技术方向，然后做一个barely work的实现，然后长期的迭代改进。从现有实现（约束） + 现有问题（目标） 生长出 具体迭代改进。这个过程同时发生在很多尺度，很多位置。不合适的，被证明是错误的实践，被删除和裁剪。

rendiation的进化代表和体现了我在编程和渲染上的理解以及具体能力的进化。在多年这样的改进过程中，我探索了渲染器工程，数据和计算框架方面的设计空间。学习和实现了诸多图形渲染上的知识。现有实现具有丰富的技术细节。在过去的每一年，我都会认为去年的代码写的很垃圾。rendiation的历史就是垃圾进化的历史。理论上我应该每年总结一次当年的开发情况，但之前几乎没有这么做。今天我认为rendiation到现在差不多整体上，方向上，终于改进/进化到了一个我觉得可以拿得出手的，不能再简单的称之为垃圾的状态。

## “团队合作”

现在整个rendiation有108k行rust代码，其中甚至几乎没有测试代码。

这个规模代码虽然看起来还好，但是逻辑密度并不低，又因为是side-project，所以有时会进入这样的状态：

- 只知道整个项目的宏观picture，和过去1周-1个月touch过的微观细节
- 有时候需要把之前写的代码当书来看，来重新探索和了解之前的想法
- 当认为某些地方需要改进时，需要重新学习之前我的怎么写的，以及之前编写的接口细节
- 曾经出现过，偶然想到一个巧妙的改进，但回到项目里发现我之前已经实现，而且做的更好
...
曾经因为在类似项目，其工程上的问题导致的痛苦经历，和令人发指的低效，让我意识到一个好的工程建设对比糟糕的工程建设，其效率会产生数量级的差别。然而在公司里，你很难说服manager在工程向的研究进行投资是必要的，而一般visible可交付的feature和效能改进才能被认可。在rendiation，我只需要关注一件事，就是工程上怎么写的爽（当然技术上需要合理）怎么来。不需要任何人认可和解释（当然实际要说服自己）。最终反而让我觉得在rendiation上的技术成果更加重要和可靠。

## “坚持”

与其说这六年来我坚持投入到这个项目，不如说这六年来我“坚持”投入到这个项目。实际上这个过程完全不需要坚持，不需要努力，主要是享受。
...
很多管理方法，都追求所谓「结果导向」。对于创新性的工作，我不认可结果导向这种模式，因为过程和结果本身是一体的。你实际上无法只关注结果而不考虑过程，你只能通过关注过程来影响结果。当你说“不管中间过程怎么怎么样，最后我只看结果”，实际上你说的是，“这件事情和我没任何关系，我只是检查最后结果”。在没有发生实际探索过程前，规划具体的结果是不可能的，甚至在出发之前，我们想要什么结果也是不确定的，更多的是这个地方很有意思，很有想象空间，很有可能又价值，所以让我们投入时间尝试一下。

创新性的工作，应该采用「过程导向」的方法论，只设定方向，而不是具体结果，改进过程的质量，去**培育**好的改变，通过优化过程本身来达成更好的结果。即便最终未能取得好的进展，中间成果和被探索过的地图本身就是有用的结果。优化过程的原则很简单，就是改进工作体验。好的「过程」，是不需要坚持的坚持。

## 未来方向

虽然现在技术上的现状进入了一个基本满意的状态，但不意味着rendiation会 *整体* *永远* 进入一个非常稳定的情况。在工程上的探索和改进会一直继续下去，因为追求稳定意味着拒绝变化的可能，拒绝变化就是拒绝改进。
