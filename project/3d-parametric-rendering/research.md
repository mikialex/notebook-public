...
- 在fragment shader拿到的uv就是该曲面点在原nurbs曲面的参数空间位置信息，然后计算是否需要被裁减，如果裁减就discard，具体见下文
...
[Direct trimming of NURBS surfaces on the GPU](https://dl.acm.org/doi/10.1145/1531326.1531353)

- per pixel 通过uv判断是否需要discard来实现裁减
  - 一般来说 判断方式和经典的2d矢量渲染一致，可能其他的算法也是可用的（比如slug），这篇采用的方法类似。
- ETER实现此方法（但是paper中说他们实现的有bug，并不是重点）
- 预处理：假设input时参数空间的nurbs曲线，转化为有理贝塞尔曲线
  - 这里可以支持任意阶的曲线，但是因为ray test n次曲线会最多有n个交点，这里的做法是按照极值来切分曲线保证单调性（ray对每个曲线段只有一个交点）
  - 曲线求交使用二分法方法（因为要支持任意阶的曲线）
  - 因为曲线段很多，所以需要加速结构

另一个需要注意的是我们总是在参数空间处理这个问题，但是input数据并一般**不是**参数空间的，所以还有额外的转化工作要做。参考：<https://stackoverflow.com/questions/28558888/nurbs-trimmed-surface>

- step ap214中面的裁减边界可以用参数空间定义（PCURVE）或者3d曲线定义（SURFACE_CURVER）。实际上大部分cad导出的step都是后者。
  - 据说cad采用surface curve是因为PCURVE 无法表示复杂的曲面间交线/精度不足
- 转化做法是离散3d curve，数值方法投影回参数空间重新分段拟合
- occ实现参考？<https://dev.opencascade.org/doc/occt-7.8.0/refman/html/classProjLib__ProjectedCurve.html#a0225bd05e5add073e640742d67e01572>

想法：

- 既然参数空间的裁减曲线大部分情况下是拟合的，那为什么不直接统一拟合到二次贝塞尔上，这样就不需要考虑什么单调性，也不需要求值曲线了，并且这样就可以直接用slug算法了。
