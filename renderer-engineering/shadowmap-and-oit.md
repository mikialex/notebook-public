...
- 基于概率的方法：存储深度cdf的多阶矩 来重建实际分布。深度和深度的平方被过滤（平均）正好就是期望和方差，n阶矩也是如此。
  - VSM：存储和记录深度（1阶矩）和深度的平方（2阶矩），然后直接使用切比雪夫不等式（切比雪夫不等式就是低阶矩的特殊情况）作为深度分布函数的估计。
  - Moment SM：就是强化版的VSM，不仅使用了期望和方差，还使用了高阶矩（>2），效果更好，成本更高
- 基于多个简单函数线性组合的方法：通过存储线性组合的系数来 来重建实际分布。因为变成了多个基函数的线性组合，那么自然的，多个分布的过滤（平均）就是其线性组合系数的过滤（平均）
  - CSM(卷积)：使用三角函数作为基函数。成本高，性价比不如其他方案，现在基本不做使用
- ESM：直接使用一个adhoc的，可以被预过滤的，形态合理的深度分布来简化问题

### 其他

很多讲解预过滤shadowmap的资料中，都会说我们通过调整depth test的方式，配合如何修改存储哪些数据来实现。这种说法非常（至少令我）困惑，因为depth test永远应该只是bool结果，实际上所谓的depth test指的是上述depth的分布函数（左移到测试深度）。另一种理解是，即便完全不过滤，1px的depth数据里存储的被认为也是过滤后的结果，所以非bool的depth test也有实际意义。深度分布函数的输出就是被遮挡的比例（概率），所以我们可以任意的修改遮挡测试的定义和解释，甚至不需要讨论某个sample是否通过测试的bool结果的概念，只要结果满足我们需要的软阴影特征即可。
...
- 固定计算开销
  ...
  - 这种表达方式支持增量的固定成本的encoding。因为场景光栅化绘制时每次只能绘制一个半透明的表面（相当于上述分布是逐步被修改和构造出来的）

### OIT的实现

- 精确方案： raytracing，完整保留上述函数
- 精确但是控制容量的方案：depth peeling，或者single pass利用atomic image。来保留函数最靠前/最重要的几个表面/转折点信息
- 近似方案：
